...
class SimpleFF(ProcessingModule):
    def __init__(self, start, stop, star_thresh=100.0):
    ...
    def run(self, frame):
        ...
        for ii, i in enumerate(
            tqdm(
                range(self.start_ind, self.stop_ind),
                total=self.stop_ind - self.start_ind,
            )
        ):
            r = data.data[i].copy()
...
class FlatFielding(ProcessingModule):
    ...
    def run(self, frame):
        ...
        ffc = (np.nanmean(mean_res, axis=0) + 0.1) - np.nanmean(mean_res)
        frame[self.out_ff] = ffc
        return frame
    ...
    def run(self, frame):
        ...
        for f in tqdm(data.data, total=len(data.data)):
            ...
            clusters.append(
                find_clusters(
                    f,
                    mean + self.upthreshold * std,
                    mean + self.lothreshold * std,
                    data.neighbors,
                )
            )
