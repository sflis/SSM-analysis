...
def compute_pixneighbor_map(
    xpix, ypix, size, pixdist: float = 1.8, intertmdist: float = 2.5
) -> list:
    ...
    for i in tqdm(range(2048), total=2048, desc="Mapping neighbor pixels"):
        for j in range(i + 1, 2048):
            ...
            dx, dy = xpix[i] - xpix[j], ypix[i] - ypix[j]
            tm1, tm2 = int(i / 64), int(j / 64)
...
class SlowSignalData:
    ...
    def __init__(
        ...
        if data.shape[0] != time.shape[0]:
            raise ValueError(
                "Number of samples ({}) must match number of timestamps ({})".format(
                    data.shape[0], time.shape[0]
                )
            )
        ...
        SlowSignalData._neighbors = self.neighbors = (
            SlowSignalData._neighbors
            or compute_pixneighbor_map(self.xpix, self.ypix, self.pix_size)
        )
