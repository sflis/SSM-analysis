...
class SteveCal:
    def __init__(self):
        ...
        # data
        self.nsb_MHz = np.array(
            [0, 6, 10, 20, 29, 38, 43, 52, 58, 90 * self.f, 100 * self.f]
        )
        self.mean_mV = np.array([0, 0, 0, 1.5, 10.5, 30.5, 56, 81, 104, 126, 146])

        self.pol = np.polyfit(
            self.nsb_MHz[self.nsb_MHz > 35], self.mean_mV[self.nsb_MHz > 35], deg=1
        )

        m = (self.nsb_MHz > 6) & (self.nsb_MHz < 30)

        self.rate_mV = interpolate.CubicSpline(
            list(self.nsb_MHz[m]) + [40],
...
class SteveCalCutoff:
    def __init__(self, mV_cutoff=3000.0):
        self.cal = SteveCal()  # super().__init__()
        self.mV_cutoff = mV_cutoff
        self.rate_cutoff = self.cal.mV2rate(self.mV_cutoff)
        self.mV2rate = pynverse.inversefunc(
            self.rate2mV, domain=[0, self.mV_cutoff + 10]
        )
...
class SteveCalSmoothCutoff:
    def __init__(self, mV_cutoff_start=1000.0):
        self.cal = SteveCal()
        ...
        self.rate_mV = interpolate.CubicSpline(
            [self.start_rate, 2000, 2850, self.end_rate],
            [mV_cutoff_start, 2970, 2999, self.cutoff_val],
            bc_type=((1, self.cal.pol[0]), (1, 0.0)),
            extrapolate=False,
        )

        self.mV2rate = pynverse.inversefunc(
            self.rate2mV, domain=[0, self.cutoff_val + 10]
        )
