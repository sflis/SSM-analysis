...
load_datafuncs = [
    load_data_factory(
        s % (f), os.path.join(root_dir, "caldata", f), "40stepsNSB", lambda x: x
    )
    for f in files
]
...
loadintensities = load_data_factory(
    "https://owncloud.cta-observatory.org/index.php/s/dcQWvybuoHQ2Eck/download",
...
def main(
    ...
    intensities = (
        loadintensities()
    )  # np.array([float(r) for r in open(input_rate_steps_file,'r').readlines()])
    ...
    for i, bc in enumerate(poshist.bincenters):
        ...
        if prev_val > 0 and cur_val == 0:
            pos_el_mapping[pos_el] = [
                pos_el_mapping[pos_el][0],
                bc + poshist.binwidths[i],
            ]
            pos_el += 1
        prev_val = cur_val
    ...
    for i in range(2048):
        if len(intensity_indicies[i]) > 0:
            ...
            x, y, err = (
                fit_points[i]["nsb_rate"],
                fit_points[i]["amp"],
                fit_points[i]["stat_err"],
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument("config", nargs="?", type=str, help="Configfile")
