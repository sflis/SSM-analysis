...
from ssm.pointing.astrometry import (rotang,
                                     rot_matrix,
                                     generate_hotspots,
                                     StarPatternMatch,
                                     matchpattern,
                                    )
matcher = StarPatternMatch.from_location(altaz_frame=altaz_frame,
                                  stars=stars,
                                  sdt=sdt,
                                  fov=12,
...
hotspots,tel_pointing,star_ind,hips_in_fov,all_hips = generate_hotspots(alt*u.rad,
                                            az*u.rad,
                                            altaz_frame,
                                            source_stars,
                                            source_star_table,
...
for mhs in matched_hs:
    ...
    plt.annotate("{}".format(int(mhs[1])),(xy[0],xy[1]),(textxy[0],textxy[1]),color='w')
