...
class StarPatternMatch:
    ...
    def identify_stars(self, hotspots, obstime=None, horizon_level=None, search_region=None,only_one_it=False):
...
def matchpattern(hotspotmap, hotspots, xlim, ylim, pattern, bins,vmag_lim=None):
    ...
    dbins = np.arange(0,np.max(hs_dist) + 0.018,0.006)#np.linspace(0, np.max(star_dist) + 0.018, 24)
    ...
    if np.sum(hs_dist_hist.bincontent * star_dist_hist.bincontent) < len(angs) * 0.7:
        ...
        for bin_i in range(1, dbins.shape[0]):
            ...
            for hotspot in hotspots[np.where(hs_bin_mask)[0]]:
                ...
                for star_pos in pattern.sp_pos[vmag_starmask,:][star_mask][np.where(star_bin_mask)[0]]:
        ...
        for bin_i in range(1, dbins.shape[0]):
            ...
            for hs_ind in np.where(hs_bin_mask)[0]:
                ...
                for star_pos_ind in np.where(star_bin_mask&cam_fov_mask)[0]:
                    ...
                    if dist<0.009:
                        matched_stars.append((hs_ind,pattern.sp_hip[star_mask][star_pos_ind]))
