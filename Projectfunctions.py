...
def get_smooth_int(data, dt, good_pixel, minutes_of_stable_int = None, interval_stable = None, plot_stable_interval = None):
    """
    ...
    if interval_stable == None:
        ...
        cal_std = [np.nanmean(cal_interval_std[i][good_pixel[0,:] == 1], axis = 0) for i in range(number_of_intervals)]
...
def calibrate_data(runfile_cal, path_, save_file = None,minutes_of_stable_int = None, interval_stable = None, plot_stable_interval = None):
    ...
    int_begin, int_end = get_smooth_int(data.data,dt,good_pixel,minutes_of_stable_int, interval_stable, plot_stable_interval)
    int_time_averaged = np.mean(intensity[[int_begin,int_end],:], axis = 0) # Mean of each pixel individually over stable int time
    int_space_averaged = np.mean(intensity[:,good_pixel[0][:]], axis = 1) # mean of all pixels at each time frame
    ...
    if save_file == True:
        ...
        calibration_properties = {"Interval_offset":(int_begin,int_end),"New_good_pixel":good_pixel,"Time_averaged_int":int_time_averaged, "Space_averaged":int_space_averaged, 
                                  "ff_coefficients_c":ff_c,"Offset_calibrated_data":offset_calibrated_data}
...
def flat_fielding(path_, runfile_ff, runfile_cal = None, calibrate = None, save_calibration_file = None, save_ff_data = None):
    ...
    if calibrate != None:
        ff_c, int_begin, int_end, int_time_averaged, int_space_averaged = calibrate_data(runfile_cal, path_, save_calibration_file)
    if calibrate == None:
        property_path = os.path.join(path_+'/'+ runfile_cal, "calibration_properties")
        with open(property_path , 'rb') as handle:
    ...
    if save_ff_data != None:
        #Save calibations
        property_path = os.path.join(path_+'/'+ runfile_ff, "Calibration_with_"+str(runfile_cal)) 
        CAL = {"Interval_offset":(int_begin,int_end),"New_good_pixel":good_pixel,"Time_averaged_int":int_time_averaged, "Space_averaged":int_space_averaged, 
                                  "Calibrated_data":F_i, "ff_coefficients_c":ff_c}                       
        with open(property_path , 'wb') as handle:
...
def compare_ffc(path_,runfile_cal_a, runfile_cal_b):
    ...
    plt.legend([r"$\frac{c_{"+str(runfile_cal_a)+"} - c_{"+str(runfile_cal_b)+"}}{c_{"+str(runfile_cal_b)+"}}$ "] , fontsize = 19, loc = 1)
    plt.axvline(0,c="k")




def plot_calibration_properties(path_,runfile_cal,save = None,camera_min = None,
                                camera_max = None, cal_min = None, cal_max = None):
    ...
    if save != None:
        plt.savefig(os.path.join(current_path+'/'+ runfile_cal, runfile_cal+"_std_interval"))
    ...
    if save != None:
        plt.savefig(os.path.join(current_path+'/'+ runfile_cal, runfile_cal+"_space_averaged_over_time"))
    ...
    if save != None:
        plt.savefig(os.path.join(current_path+'/'+ runfile_cal, runfile_cal+"_camera_time_averaged"))
    ...
    if save != None:
        plt.savefig(os.path.join(current_path+'/'+ runfile_cal, runfile_cal+"_calibrated_data"))
    ...
    if save != None:
        plt.savefig(os.path.join(current_path+'/'+ runfile_cal, runfile_cal+"_flat_field_coeffs_c"))
    ...
    if save != None:
        plt.savefig(os.path.join(current_path+'/'+ runfile_cal, runfile_cal+"_offset_calibrated_data"))
